...
OPTIONS:
  ...
  --tweaks                Specify versions for tweaks
                          1. [neo|trinity]	Neo|Trinity| ColorSchemes version
	...
	--tweaks)
		...
		for variant in $@; do
			...
			trinity)
				trinity="true"
				ctype="-Trinity"
				echo -e "Trinity ColorScheme version! ..."
				shift
				;;
			neo)
				neo="true"
				ctype="-Neo"
				echo -e "Neo ColorScheme version! ..."
				shift
				;;
			black)
				blackness="true"
...
trinity_color() {
	sed -i "/\@import/s/color-palette-default/color-palette-trinity/" "${SRC_DIR}/sass/_tweaks-temp.scss"
	sed -i "/\$colorscheme:/s/default/trinity/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}

neo_color() {
	sed -i "/\@import/s/color-palette-default/color-palette-neo/" "${SRC_DIR}/sass/_tweaks-temp.scss"
	sed -i "/\$colorscheme:/s/default/neo/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}
